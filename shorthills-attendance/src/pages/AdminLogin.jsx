...
export default function AdminLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    if (email !== adminEmail) {
      setError('This login is for admin only. Employees use the Employee Login.');
      return;
    }
    setLoading(true);
    try {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-navy-800 border border-navy-700 rounded-2xl p-8">
          ...
          <p className="text-center text-gray-500 text-sm mt-6">
            Employee?{' '}
            <a href="/login" className="text-electric-400 hover:text-electric-300 transition-colors">Go to Employee Login</a>
          </p>
        </div>
