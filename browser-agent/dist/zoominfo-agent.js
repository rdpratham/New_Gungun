...
async function getContactData(page, contact) {
...
async function saveResults(contacts) {
    // Save JSON
    ...
    const rows = contacts.map((c, i) => [i + 1, c.company, c.name, c.title, c.level, c.email, c.directPhone, c.mobilePhone, c.notes]
